...
class Solution(object):
    ...
    ###################################################################################################################
    ###################################################################################################################
    def longestConsecutive_nonGraphSolution(self, nums):
        """
        :type nums: List[int]
        :rtype: int

        Task: find the maximum length of a subset of consecutive numbers
        """
        if nums == []: return 0
        ...
        for num in nums_set:
            if num not in seen:
                ...
                while queue:
                    ...
                    seen.add(curr)
                    curr_len += 1
                    if curr+1 not in seen and curr+1 in nums_set:
                        queue.append(curr+1)
                    if curr-1 not in seen and curr-1 in nums_set:
                        queue.append(curr-1)
                max_len = max(curr_len, max_len)
